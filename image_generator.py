...
NEON_GREEN  = "#3DFF9A"   # texto bottom izquierda (mint Bet365)
...
def _load_font(size: int, weight: str = "regular") -> ImageFont.FreeTypeFont:
    ...
    HelveticaNeue.ttc indices:
      0 = Regular, 1 = Bold, 4 = Medium (usado en botones Bet365)
    """
    idx_map = {"regular": 0, "bold": 1, "medium": 4}
    hv = "/System/Library/Fonts/HelveticaNeue.ttc"
    try:
        return ImageFont.truetype(hv, size, index=idx_map.get(weight, 0))
    ...
    for path in fallback.get(weight, fallback["regular"]):
        try:
...
def generate_bet365_card(
    ...
    font_odd = _load_font(26, weight="bold")
    odd_str  = f"{odd:.2f}"
    ...
    font_btn  = _load_font(24, weight="medium")   # medium = idx 4, igual que Bet365
    btn_label = "Set Stake"
    ...
    font_url  = _load_font(24, weight="medium")   # mismo peso que Set Stake
    url_label = "Place Bet"
